...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QScrollArea, QFrame, QLineEdit
...
SERPAPI_API_KEY = 'b35b6d8295044cf38126f3381d4e7b526014a6038bbea22f4cc078ad88367f19'
...
def is_real_time_query(query):
    real_time_keywords = [
        ...
        'update', 'news', 'stock', 'temperature', 'traffic', 'forecast','playing',
        'match', 'standings', 'alert', 'report', 'happening', 'breaking','2024',
        'event', 'now', 'recent', 'trending', 'live feed', 'scorecard','current','currently',
        ...
        'up-to-date', 'emergency', 'news flash', 'quick update', 'real-time news','recently','future','acting',
        'up-to-the-minute', 'breaking news', 'instant update', 'real-time feed', 'moment-to-moment','now','till date','suggest'
    ]
...
class VoiceAssistantApp(QWidget):
    ...
    def initUI(self):
        layout = QVBoxLayout()
        self.status_label = QLabel("Click 'Start Listening' to begin or type your command", self)
        ...
        self.text_input.returnPressed.connect(self.processTypedCommand)  # Connect to function to process typed input
        ...
        # New "Stop Generating" button
        self.stop_generating_button = QPushButton("Stop Generating", self)
        self.stop_generating_button.clicked.connect(self.stopGenerating)
        button_layout.addWidget(self.stop_generating_button)
    ...
    # New "Stop Generating" function
    def stopGenerating(self):
        # Stop both the speech and listening threads
        if self.listening_thread and self.listening_thread.isRunning():
            self.listening_thread.terminate()
            self.updateStatus("Stopped generating", "stopped")
    ...
    # New function to process typed commands
    def processTypedCommand(self):
        typed_query = self.text_input.text()
        if typed_query:
            self.updateStatus(f"You typed: {typed_query}", "user")
            self.addConversationBox(typed_query, "user")
            response = self.getResponseForQuery(typed_query)  # Handle typed input as with voice input
            self.updateStatus(response, "assistant")
            self.addConversationBox(response, "assistant")
            engine.say(response)
            engine.runAndWait()
            self.text_input.clear()

    def getResponseForQuery(self, query):
...
# Don't use stop listening and stop response just use start listening and stop generating
# Don't give another command while the ai model is speaking